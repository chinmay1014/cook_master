...
import { Loader2, Plus, RefreshCw, Save, Trash2, Calendar, ShoppingCart, Info, Clock, Flame, Users, Share2, Search, Edit2, MessageCircle } from 'lucide-react';
...
// Helper to extract time in minutes from a string
...
const FoodChat: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [messages, setMessages] = useState<Message[]>([{ role: 'assistant', content: 'Hello! I am the Master Cook Expert SLM. I can help with recipes, ingredient substitutions, cooking techniques, or nutritional advice. How can I assist you today?' }]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const prompt = `You are the Master Cook Expert SLM, a specialized assistant for all things food. Be concise, expert, and focused on cooking, recipes, ingredients, and nutrition. User query: ${input}`;
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 max-w-[90vw] glass-panel rounded-2xl shadow-2xl flex flex-col h-[500px] z-50 border border-white/30">
      <div className="p-4 border-b border-white/20 flex justify-between items-center">
        <h3 className="font-display font-bold text-lg text-gray-800 dark:text-white">Master Cook SLM</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, i) => (
          <div key={i} className={`p-3 rounded-xl ${msg.role === 'user' ? 'bg-orange-100 dark:bg-orange-800 ml-auto' : 'bg-gray-100 dark:bg-gray-700'}`}>
            <p className="text-sm">{msg.content}</p>
          </div>
        ))}
        {isLoading && <div className="text-sm p-3 bg-gray-100 dark:bg-gray-700 rounded-xl">Thinking...</div>}
      </div>
      <div className="p-4 border-t border-white/20">
        <div className="flex gap-2">
            <input 
                ...
                className="flex-1 p-2 rounded-lg bg-white/50 dark:bg-gray-800 border"
                placeholder="Ask me anything..."
            />
            <button onClick={sendMessage} className="p-2 bg-orange-500 text-white rounded-lg">Send</button>
        </div>
      </div>
    </div>
  );
...
type Timer = { id: string, stepIndex: number, label: string, timeLeft: number, isActive: boolean };
...
const RecipeDisplay: React.FC<RecipeDisplayProps> = ({ recipe, onModify, onScale, isLoading, onSave, isFavorite, onAddToMealPlan, onEditRecipe }) => {
  ...
  const startTimer = (stepIndex: number, durationMinutes: number) => {
    const newTimer: Timer = {
      id: `${stepIndex}-${Date.now()}`,
      stepIndex,
      label: `Timer for step ${stepIndex + 1}`,
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2">
            ...
            {recipe.cookTime && (
                <span className="text-xs font-semibold px-2 py-1 bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 rounded-full border border-red-200 dark:border-red-800/50">
        ...
        <div className="md:col-span-7">
          ...
          <ol className="space-y-3 list-decimal list-inside text-gray-600 dark:text-gray-300">
            {recipe.instructions.map((step, index) => {
              ...
              return (
                <li key={index} className="pl-2 group flex items-start justify-between">
                  <span>{step}</span>
                  {minutes && (
                    <button
                      onClick={() => startTimer(index, minutes)}
                      className="ml-2 px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/50 dark:hover:bg-blue-800/50 text-blue-800 dark:text-blue-200 rounded transition-colors flex-shrink-0"
                    >
          ...
          {timers.length > 0 && (
            <div className="mt-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-200 dark:border-orange-800/50">
              <h4 className="font-semibold text-orange-800 dark:text-orange-200 mb-3 flex items-center gap-2">
                <Clock className="w-4 h-4" /> Active Timers
              </h4>
              <div className="space-y-2">
                {timers.map(timer => (
                  <div key={timer.id} className="flex items-center justify-between bg-white dark:bg-orange-950/30 p-2 rounded-lg border border-orange-100 dark:border-orange-800/30">
                    <span className="text-sm font-medium">{timer.label}</span>
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-mono ${timer.timeLeft === 0 ? 'text-red-500 font-bold' : ''}`}>
                        {Math.floor(timer.timeLeft / 60)}:{(timer.timeLeft % 60).toString().padStart(2, '0')}
                      </span>
                      <button onClick={() => toggleTimer(timer.id)} className="p-1 hover:bg-orange-100 dark:hover:bg-orange-800 rounded">
                        {timer.isActive ? 'Pause' : 'Resume'}
                      </button>
                      <button onClick={() => removeTimer(timer.id)} className="p-1 hover:bg-red-100 dark:hover:bg-red-900 rounded text-red-500">
...
const ShoppingList: React.FC<ShoppingListProps> = ({ mealPlan }) => {
  ...
  const parseIngredient = (str: string): { quantity: number, unit: string, name: string } => {
    ...
    if (match) {
        ...
        return {
            ...
            name: match[3] ? match[3].toLowerCase() : trimmed
        };
    }
  ...
  const finalIngredients = Object.values(ingredientMap).map(i => {
      const quantityStr = i.quantity % 1 === 0 ? i.quantity.toString() : i.quantity.toFixed(1).replace(/\.0$/, '');
      return `${quantityStr} ${i.unit} ${i.name}`.trim();
  }).sort();
  ...
  return (
    <HashRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      <div className="min-h-screen bg-transparent text-gray-900 dark:text-gray-100 font-sans p-4 sm:p-6 lg:p-8">
        ...
        {isChatOpen && <FoodChat onClose={() => setIsChatOpen(false)} />}
        <button
          onClick={() => setIsChatOpen(!isChatOpen)}
          className="fixed bottom-6 left-6 p-4 bg-orange-600 text-white rounded-full shadow-lg hover:bg-orange-700 z-50"
        >
          <MessageCircle />
        </button>
      </div>
